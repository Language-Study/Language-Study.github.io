...
function getAvatarUrl(filename) {
    return `icons/avatars/${filename}`;
}

async function saveUserAvatar(avatarFile) {
...
async function loadUserAvatar() {
    if (!currentUser) return AVATAR_LIST[0];
    try {
        const doc = await db.collection('users').doc(currentUser.uid).collection('metadata').doc('settings').get();
        if (doc.exists && doc.data().avatar && AVATAR_LIST.includes(doc.data().avatar)) {
...
function updateAvatarUI(avatarFile) {
    ...
    document.querySelectorAll('#avatarPicker .avatar-option').forEach(img => {
        if (img.getAttribute('data-avatar') === avatarFile) {
...
async function initAvatarPicker() {
    ...
    if (!picker) return;
    const avatarFile = await loadUserAvatar();
    updateAvatarUI(avatarFile);
    picker.querySelectorAll('.avatar-option').forEach(img => {
        img.addEventListener('click', async () => {
            const selected = img.getAttribute('data-avatar');
            await saveUserAvatar(selected);
            updateAvatarUI(selected);
        });
...
// On auth state change, load avatar and update UI
if (typeof onAuthStateChanged === 'function') {
    onAuthStateChanged(async (user) => {
        if (!user) return;
        const avatarFile = await loadUserAvatar();
        updateAvatarUI(avatarFile);
        setTimeout(initAvatarPicker, 500); // Wait for modal to render
    });
}
